...
const WHATSAPP_SUPPORT_URL = String(process.env.NEXT_PUBLIC_SUPPORT_WHATSAPP_URL || "https://wa.me/233555596082").trim();

function whatsappEscalationReply(order) {
  if (!order) return null;

  // Only escalate orders where payment is confirmed but fulfillment is still
  // unresolved. An old unpaid/abandoned order must never trigger the WhatsApp
  // path simply because six hours have elapsed.
  const fulfillmentStatus = String(order.fulfillmentStatus || "").toLowerCase();
  const paidUnresolved =
    String(order.status || "").toLowerCase() === "payment_verified" ||
    ["processing", "queued_with_provider", "manual_review"].includes(fulfillmentStatus);
  if (!paidUnresolved) return null;

  // Start the six-hour clock from payment verification when available. Fall
  // back to creation time for older records that predate paymentVerifiedAt.
  const verifiedAt = order.paymentVerifiedAt || order.payment_verified_at;
  const createdAt = verifiedAt || order.createdAt || order.created_at;
  ...
  const stateText = fulfillmentStatus === "queued_with_provider"
    ? "is still queued with the provider"
    : fulfillmentStatus === "manual_review"
      ? "is under manual review"
      : "is still being processed";

  if (WHATSAPP_SUPPORT_URL) {
    return "I'm sorry this has taken longer than expected. Your paid order" + reference + " " + stateText + " and has remained unresolved for more than 6 hours. At this point, please contact our support team directly on WhatsApp so we can follow up with you personally. You can reach us here: " + WHATSAPP_SUPPORT_URL + ". Please include your order reference and don't make another payment.";
  }
  return "I'm sorry this has taken longer than expected. Your paid order" + reference + " " + stateText + " and has remained unresolved for more than 6 hours. Please contact our support team directly on WhatsApp so we can follow up with you personally. Please include your order reference and don't make another payment.";
}

export function orderStatusReply(order) {
  if (!order) return null;
  const whatsapp = whatsappEscalationReply(order);
  ...
  if (status === "payment_pending") return "I've checked your order." + reference + " Your payment is still being confirmed by Paystack. Please don't make another payment or place a duplicate order while we check it.";
  if (status === "payment_failed") return "I've checked your order." + reference + " Paystack did not confirm this payment. Please don't make another payment from the same order reference; start a new checkout only after confirming the previous attempt did not succeed.";
...
const SELLING_RULE_PATTERNS = {
  ...
  airtelTigoPrefix: ["026", "056", "027", "057", "airteltigo prefix", "airtel tigo number"],
...
export function sellingRuleReply(text) {
  ...
  if (matchesAny(t, SELLING_RULE_PATTERNS.airtelTigoPrefix) && (t.includes("valid") || t.includes("prefix") || t.includes("number") || t.includes("at "))) {
    return "For AT iShare and AT BigTime, our Delivery & selling rules require an AirtelTigo number. The site uses network-prefix checks as a safety hint; the number may have been ported, so confirm the recipient's current network before paying. If you've already paid and the number was not eligible, please don't place another order; send the order reference through Feedback & complaints so support can review it.";
  }
...
export function supportReply(text, order = null) {
  ...
  const asksForStatus = matchesAny(t, [
    "track",
    "status",
    "where is my order",
    "check my order",
  ...
  // Verified order status is deterministic so the assistant never guesses
  // about a real transaction.
  if (order && asksForStatus) return orderStatusReply(order);

  // Security-sensitive requests remain deterministic.
  const asksForSecret = (secretWords) =>
    secretWords.some((secret) => t.includes(secret)) &&
    ["send", "share", "give", "provide", "need", "enter", "tell", "type"].some((action) => t.includes(action));

  if (
    asksForSecret(["pin", "otp", "one-time code", "one time code", "password", "passcode", "cvv", "cvc", "card number"])
  ) {
    return "Please don't send your PIN, OTP, password, CVV or card details in chat. Use the secure Paystack checkout or the Feedback & complaints page instead.";
  }

  // Normal conversation is deliberately handed to Gemini so complaints,
  // questions, greetings and back-and-forth do not sound like canned macros.
  return null;
}
...
const GREETING_REPLY_FIRST = "Hello! 😊 I'm Annette from PjDigitalServices. I'm here to help. What would you like to do today — make a purchase, check your order, or resolve an issue?";
