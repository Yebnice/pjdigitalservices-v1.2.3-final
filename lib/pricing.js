...
// Data Top-up flow intentionally have an explicit GHS 0 margin.
...
export const DEFAULT_BUSINESS_MARGIN_FIXED_GHS = Number.isFinite(Number(process.env.DEFAULT_BUSINESS_MARGIN_FIXED_GHS))
  ? Math.max(0, Number(process.env.DEFAULT_BUSINESS_MARGIN_FIXED_GHS))
  : 1;
...
// - All other services/tier products use a default fixed GHS 1.00 business
//   margin unless an explicit service-specific rule overrides it.
...
export function businessMarkup(baseAmount, { orderType, network } = {}) {
  ...
  // Default policy is a fixed GHS 1.00 margin. Percentage margins are used
  // only when explicitly configured for a specific service.
  const percent = hasExplicitPercent
    ? Math.max(0, Math.min(100, Number(rule.percent) || 0))
    : 0;
  const fixed = hasExplicitFixed
    ? Math.max(0, Number(rule.fixed) || 0)
    : (hasExplicitPercent ? 0 : DEFAULT_BUSINESS_MARGIN_FIXED_GHS);
