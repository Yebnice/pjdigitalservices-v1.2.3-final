...
function sanitizePublicResult(result) {
  ...
  // "checkers" was missing here: BECE/WASSCE result-checker purchases (and
  // the paid lookup service) come back from Techlink as
  // { checkers: [{ serialNumber, pin, type, ... }] } — see
  // POST /result-checker/purchase and /result-check-service/request in the
  // docs. Techlink's own request body never includes the customer's email
  // or phone, so its own "deliveryMethod: email|sms" cannot reach the
  // customer — it can only be going to this account's own Techlink login.
  // Without "checkers" in this allowlist, a customer who paid for a
  // voucher never saw their serial/PIN anywhere: not on the receipt (this
  // is what it reads from), not in the API response, and (before the
  // matching fix in notifyCustomerOrderFulfilled) not in the delivery
  // email either — paid for, "delivered", and never actually handed over.
  const allowed = ["token", "units", "reference", "transactionId", "message", "status", "customerName", "package", "amount", "amountDue", "checkers"];
  const out = {};
...
export function toPublicOrder(order) {
  ...
  return {
    ...
    businessMarkupAmount: order.businessMarkupAmount,
    status: order.status,
...
// the admin can see exactly what came back. Retrying a bulk order resubmits
// the WHOLE batch, so the note says to verify each recipient first.
...
export async function claimStaleQueuedAlerts(staleMinutes = 180, limit = 20) {
  ...
  }).in("reference", references).eq("fulfilled", false).eq("fulfillment_status", "queued_with_provider").is("queued_alert_sent_at", null).select();
  if (error) throw new Error(error.message);
  return (data || []).map(fromRow);
}
...
export async function promoteExhaustedFailedOrders(limit = 20) {
  ...
  const { data, error } = await supabase.from("orders").update({
    ...
    manual_review_at: reviewAt,
    urgent_review_notified_at: null,
  }).in("reference", references).eq("fulfilled", false).eq("fulfillment_status", "failed").is("manual_review_at", null).select();
  if (error) throw new Error(error.message);
  return (data || []).map(fromRow);
}
...
export async function manuallyResolveOrder(reference, action, note) {
  ...
  if (action === 'confirm_fulfilled') {
    ...
    }).eq("reference", reference).eq("fulfilled", false).in("fulfillment_status", ["manual_review", "failed", "queued_with_provider"]).select().maybeSingle();
    if (error) throw new Error(error.message);
    return data ? fromRow(data) : null;
  }
  ...
  if (action === 'retry') {
    ...
    }).eq("reference", reference).eq("fulfilled", false).in("fulfillment_status", ["manual_review", "failed"]).select().maybeSingle();
    if (error) throw new Error(error.message);
    return data ? fromRow(data) : null;
  }
...
export async function listManualReviewOrders(limit = 50) {
  ...
  const { data, error } = await supabase.from("orders").select("*").eq("fulfilled", false).in("fulfillment_status", ["manual_review", "failed"]).order("created_at", { ascending: true }).limit(limit);
  if (error) throw new Error(error.message);
  return (data || []).map(fromRow);
}
