...
async function tlFetch(path, options = {}) {
  ...
  if (res.status >= 500) {
    throw new TechlinkAmbiguousError(data.message || data.error || `Techlink server error (${res.status}): ${path}`);
  }
  if (!res.ok || data.success === false) {
    throw new Error(data.message || data.error || `Techlink request failed (${res.status}): ${path}`);
  }
  return data;
}
...
export async function lookupEcgMeter(meter) {
  return tlFetch(`/ecg/lookup?meter=${encodeURIComponent(meter)}`); // { customerName, district }
}
...
export async function validateWaterMeter({ account }) {
  return tlFetch("/korba/validate", {
    method: "POST",
    body: JSON.stringify({ billType: "WATER", account }),
  }); // { accountName, amountDue, ... } — confirm exact field names against a live response
}
...
export async function registerAfa({ network, fullName, ghanaCard, phone, dob, region, location, occupation }) {
  return tlFetch("/afa/register", {
    method: "POST",
    body: JSON.stringify({
      // Techlink's docs disagree with themselves. Two of the three places
      // (the prose "all of fullName, phone, ghanaCard, dob ... are required",
      // and the request-body definition) use fullName/ghanaCard/dob; only the
      // auto-generated curl example uses name/idNumber/dateOfBirth. This code
      // used to send ONLY the curl names. Until a live/test call settles which
      // is real, send both spellings — unknown extra keys are normally
      // ignored. (A tlg_test_ key returns a validated, simulated response
      // without debiting the wallet, so this can be checked safely.)
      name: fullName,
      fullName,
      phone,
      idNumber: ghanaCard,
