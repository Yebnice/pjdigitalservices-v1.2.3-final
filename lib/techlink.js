...
export async function lookupEcgMeter({ meter } = {}) {
  ...
  if (!requestedMeter) {
    const err = new Error("ECG meter number is required");
    err.status = 400;
    throw err;
  }

  // Techlink documents /ecg/lookup as a meter lookup. Phone is an optional
  // alternate lookup parameter for listing meters associated with a number;
  // it is not required when resolving a specific meter.
