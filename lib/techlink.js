...
export async function lookupEcgMeter({ meter, phone } = {}) {
  ...
  if (!requestedMeter || !requestedPhone) {
    const err = new Error("ECG meter number and telephone number are both required");
    err.status = 400;
    throw err;
  }

  // PjDigitalServices ECG verification requires the meter and telephone
  // number together before the customer can proceed to payment — that's
  // enforced above regardless of what Techlink's API does with the params.
  //
  // Techlink's docs only show GET /ecg/lookup with a `meter` param, and
  // describe `phone` as an *alternative* way to call it ("pass phone
  // instead to list meters on a number"), not something documented to be
  // sent together with meter. Rather than assume the combined query works,
  // try it first (extra query params are usually just ignored by REST
  // APIs), but fall back to the documented meter-only call if the combined
  // attempt errors or fails to resolve — so a stricter-than-expected API
  // on Techlink's side can't block every ECG top-up. normalizeEcgLookupResponse
  // still matches the result back to the requested meter, so a phone-keyed
  // list response is handled safely either way.
  const params = new URLSearchParams({ meter: requestedMeter, phone: requestedPhone });

  let data;
  let combinedFailed = false;
  try {
    data = await tlFetch(`/ecg/lookup?${params.toString()}`);
  } catch (err) {
    if (Number(err?.status) === 404) throw err; // a real "not found" — no point retrying
    combinedFailed = true;
  }

  let resolved = combinedFailed ? null : normalizeEcgLookupResponse(data, requestedMeter);

  if (!resolved) {
    const meterOnlyParams = new URLSearchParams({ meter: requestedMeter });
    const fallbackData = await tlFetch(`/ecg/lookup?${meterOnlyParams.toString()}`);
    resolved = normalizeEcgLookupResponse(fallbackData, requestedMeter);
  }
