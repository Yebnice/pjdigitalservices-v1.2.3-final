...
export async function verifyAndPrepareOrder(reference) {
  ...
  if (txn.status !== "success") {
    ...
    if (terminalPaymentFailure) {
      const failedOrder = await markPaymentFailed(reference, paymentStatus || "payment_failed");
  ...
  if (txn.currency && txn.currency !== "GHS") {
    const failedOrder = await markPaymentFailed(reference, "currency_mismatch");
    return { kind: "failed", status: "currency_mismatch", order: failedOrder };
  }
  ...
  if (Number(txn.amount) !== expectedPesewas) {
    const failedOrder = await markPaymentFailed(reference, "amount_mismatch");
    return { kind: "failed", status: "amount_mismatch", order: failedOrder };
  }
